	...
	for withIssues || withPRs {
		...
		if withIssues {
			if data.Repository.Issues == nil {
				...
				withIssues, issuesAfter = nextPage(data.Repository.Issues.PageInfo.HasNextPage, data.Repository.Issues.PageInfo.EndCursor, issuesAfter)
			}
		}
		if withPRs {
			if data.Repository.PullRequests == nil {
				...
				withPRs, prsAfter = nextPage(data.Repository.PullRequests.PageInfo.HasNextPage, data.Repository.PullRequests.PageInfo.EndCursor, prsAfter)
			}
		}
	}
	if defaultBranch != "" {
...
// nextPage decides whether pagination continues, guarding against upstream data that could spin the
// loop forever: hasNextPage=true with a missing or stagnant (unchanged) cursor is treated as done
// rather than as "keep going" (never panic/never hang on upstream data, arc42 §8.7).
func nextPage(hasNextPage bool, endCursor string, prevAfter any) (bool, any) {
	if !hasNextPage || endCursor == "" {
		return false, prevAfter
	}
	if prev, ok := prevAfter.(string); ok && prev == endCursor {
		return false, prevAfter
	}
	return true, endCursor
}
