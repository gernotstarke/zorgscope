...
logo: ## Regenerate the static logo assets from docs/logo/zorgscope-logo-sheet.jpeg
	...
	@# 128 px, not 256: the only thing that renders logo.png is the page header, at 36 px inside
	@# the scanning orbit, and 128 covers a 3x display with a little to spare. PNG does not gzip,
	@# so every byte of it is a byte on the wire against QS-2.3 — 256 px would spend seven
	@# kilobytes to draw a 36 px mark.
	docker run --rm --entrypoint magick -v "$(CURDIR)":/w -w /w $(IMAGEMAGICK) /w/.logo-master.png \
	  -resize 128x128 -colors 32 -strip -define png:compression-level=9 internal/web/static/logo.png
	docker run --rm --entrypoint magick -v "$(CURDIR)":/w -w /w $(IMAGEMAGICK) /w/.logo-master.png \
