...
| Id | Context | Stimulus | Response | Measure |
|----|---------|----------|----------|---------|
| QS‑2.3 | The representative configuration (10 repositories, ~150 open items between them) | The dashboard is rendered, unfiltered | The response is small enough for a slow connection | ≤ 150 kB uncompressed HTML plus ≤ 50 kB of static assets on the wire (gzip-negotiated), including the vendored htmx. `TestRenderedPageStaysInsideItsBudget` and `TestStaticAssetsFitTheirBudgetOnTheWire` (`internal/web/dashboard_test.go`) assert both halves of the budget against the rendered fixture. |
