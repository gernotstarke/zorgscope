...
| 0003 | Fly.io scaled to zero, refreshed by an external cron trigger | `0003-fly-scale-to-zero-external-cron.md` | partially superseded by 0010 (cron half only; scale to zero stays) |
| 0004 | Turso and libSQL for persistence, `libsql-server` locally | `0004-turso-libsql.md` | superseded by 0010 |
| 0005 | Embedded SQL migrations rather than a schema tool | `0005-embedded-sql-migrations.md` | superseded by 0010 |
| 0006 | First-seen versus last-visit as the definition of "new" | `0006-first-seen-versus-last-visit.md` | superseded by 0010 |
| 0007 | Token sign-in with a session cookie derived from the token | `0007-token-sign-in-derived-cookie.md` | superseded by 0009 |
...
| 0014 | Security from evidence already fetched: two tiers, no security API | `0014-security-from-evidence-already-fetched.md` | accepted |
...
of 2026-09-15 (`docs/superpowers/specs/2026-09-15-stateless-reset-design.md`; the first two were
removed in the stateless reset, see git history) — is followed by a record for each decision it
made, so that the reasoning can be read on its own rather than excavated from the document that
happened to occasion it.
