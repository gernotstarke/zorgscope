...
| D‑14 | 15 | The plan's app‑layer log lines use ad‑hoc field names (`source`, `items`, `duration`) | `internal/app` and `internal/server` use `component`, `source_id`, `duration_ms` (integer milliseconds), `status` throughout | arc42 §8.8 fixes these as the log vocabulary; structured logs are only useful when a field means the same thing in every component, and `duration_ms` names its unit where a bare `duration` does not. |
...
| D‑16 | — | FR‑2.4 AC2: items with no activity for ≥ 30 days are "additionally marked STALE and listed in a collapsed section" | Not implemented in M1. `domain.LevelStale` exists and sorts below `LevelExpiring`, so `NeedsAttention()` never selects it and its `"STALE"` badge is currently unreachable — deliberately, not accidentally. The collapsed section does not exist. | Belongs with M4's filtering work, not M1's walking skeleton. Recorded here (whole‑branch review finding) rather than left as a silent gap; see the comment at `LevelStale` in `internal/domain/attention.go`. |
| D‑17 | — | The logger path redacts secrets automatically (`internal/logging`, QS‑3.3); `internal/app/scheduler.go`'s `st.ErrorMsg = err.Error()` has no equivalent guard, so an upstream error string flows unredacted to SQLite, the header tooltip, and `/status` JSON | Left unchanged for M1 — verified safe by two independent audits (no GitHub adapter error interpolates the token) — but recorded as a known structural asymmetry deferred to M2, when three more adapters land on this unguarded side. The eventual fix is a `redact(err.Error(), cfg.Secrets)` helper applied once at that assignment. | Whole‑branch review finding: the logger path being defended while this path rests on per-adapter human review does not scale past a single adapter, and M2 adds three. See the comment at that assignment in `internal/app/scheduler.go`. |
